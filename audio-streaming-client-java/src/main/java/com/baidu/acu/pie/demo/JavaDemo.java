...
public class JavaDemo {
    ...
    public void asyncRecognition() {
        String longAudioFilePath = "testaudio/300s.wav";
        AsrClient asrClient = createAsrClient();
        CountDownLatch finishLatch = new CountDownLatch(1);

        StreamObserver<AudioFragmentRequest> sender = asrClient.asyncRecognize(
                // 这个回调函数用来处理每一个fragment的识别结果
                it -> {
                    System.out.println(
                            Instant.now().toString() + "\t" + Thread.currentThread().getId()
                                    + " receive fragment: " + it);
                }, finishLatch);
