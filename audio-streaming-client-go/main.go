...
func generateInitRequest() protogen.InitRequest {
	...
	content.Token = util.HashToken(username, password, nowTime)
	return content
}
...
func main() {
	flag.Parse()
	// 处理音频文件
	client.ReadFile(*serverAddr, audioFile, sampleRate, generateInitRequest())
	// 处理麦克风音频流
	//client.ReadMicrophone(*serverAddr, sampleRate, generateInitRequest())
}
